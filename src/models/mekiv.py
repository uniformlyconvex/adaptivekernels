...
class XModel(torch.nn.Module):
    ...
    def compute_good_indices(self, labels: torch.Tensor) -> torch.Tensor:
        ...
        indices = (
            torch.abs(label_real - mu_real) <= no_stds * std_real
        ).logical_and(
    ...
    def forward(self):
        gamma_X = torch.linalg.solve(
            self.K_Z1Z1 + self.n * torch.exp(self.log_lambda_X) * torch.eye(self.n, device=self.device),
            self.K_Z1Z2
        )
    ...
    def loss(self, good_indices_only: bool = False):
        ...
        mse = torch.mean(
            torch.norm(preds - truth, dim=-1) ** 2
        )

        msd = torch.mean(
            torch.norm(self.X - (self.M + self.N)/2, dim=-1) ** 2
        )
        # reg = 1000*msd
    ...
    def fit(self, no_epochs=1000, good_indices_only: bool = True):
        ...
        for epoch in tqdm(range(no_epochs)):
            if epoch == 0:
                ...
                print(f"    Mean square distance to | True X: {ms_X:.4f} | M: {ms_M:.4f} | N: {ms_N:.4f} | MN: {ms_MN:.4f}")
            ...
            if epoch % (no_epochs / 100) == 0:
                if self.true_X is not None:
                    ...
                    print(f"Epoch {epoch} | Loss: {loss.item():.4f} | Good indices: {good_indices_only}")
                    print(f"    Mean square distance to | True X: {ms_X:.4f} | M: {ms_M:.4f} | N: {ms_N:.4f} | MN: {ms_MN:.4f}")
                else:
...
class MEKIV:
    ...
    def __post_init__(self):
        ...
        if self.real_X is not None:
            first, second = misc.rand_split((self.M, self.N, self.Y, self.Z, self.real_X), p=0.50)
        ...
        self.M_kernel = gaussian.MultiDimGaussianKernel(self.M_lengthscales)
    ...
    def stage_1_tuning(self,
        ...
        def get_gamma_Z2(lmbda: float) -> torch.FloatTensor:
            gamma_Z2 = torch.linalg.solve(
                K_Z1Z1 + lmbda * n * torch.eye(n, device=K_Z1Z1.device),
                K_Z1Z2
            )
    ...
    def train(self):
        ...
        alpha_samples = self.N_kernel.sample_from_bochner(5000)
        ...
        self.X_kernel = gaussian.MultiDimGaussianKernel(misc.auto_lengthscales(self.fitted_X1))
        K_X1X1 = self.X_kernel(self.fitted_X1, self.fitted_X1)

        W = K_X1X1 @ torch.linalg.solve(
            self.K_Z1Z1 + self.n * self.lambda_X * torch.eye(self.n),
            self.K_Z1Z2
        )
